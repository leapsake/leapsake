...
describe("onboarding reminders (end to end through core)", () => {
  it("seeds the day-one nudges on a fresh store", async () => {
    ...
    // Home order (through the real driver + list ordering): protecting the device
    // sits above importing into it.
  ...
  it("retires 'import your contacts' on people.create with no explicit reconcile", async () => {
    ...
    // The account invitation is untouched (this store still holds none).
    expect(live.map((r) => r.id)).toContain(idFor("create-account"));
  });
  ...
  it("coexists with a birthday reminder in one reconcile (neither family prunes the other)", async () => {
    ...
    // import nudge, so we expect the account nudge + the birthday reminder.
