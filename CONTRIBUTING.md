...
These are the lens every testing decision is judged against. Each one _cuts options_ rather
...
2. **Match production as closely as possible.** Signal strength scales with runtime fidelity;
   a different engine, bundler or module graph than ships is a _smell_. This one rule rejects
   both mocked SQLite and WASM SQLite for the mobile driver — see
   [`apps/mobile/README.md`](apps/mobile/README.md) → _Why the driver test needs a device_.
3. **Test what's observable to the consumer.** Assert only on the surface the thing's
   _consumer_ sees — a port's return values, a package's public API, text and pixels on
...
shared and driver-injected, we do _not_ re-prove it per platform. We prove the **driver** is
...
> _automated and green_ on the closest approximation of that platform.**

A **simulator/emulator/VM is itself the accepted approximation** — the gate does not require
real hardware or a device cloud. "Closest approximation" means the closest _automatable_
...
**The rule's unit is _platform × rung_, not platform.** The full catalog is required before
...
| `beta`  | external TestFlight — the first strangers                 | an evening of typing, and only if they ignored the notes | catalog Flows **1, 2 (the smoke) and 4**, **Flow 4's two at-rest door acts included**, **on-screen assertions only**. The `e2e` tier is `ready` and **passes** under `--strict`                                                                                                                  |
...
rung before anyone keeps real records here. Flow 4's door acts gate from `beta` _(owner, 2026-09-24)_:
...
and that sentence is only true while the tester has been _told_ it is a beta and not a vault.
...
merely _skipped_ would defeat the whole thing. Every flow also stays non-vacuous by sabotage,
...
the app through OS/UI, and is what we own and keep. The **execution layer** — _where_ a harness
...
Large commits create more errors and are harder to revert. Say _why_ in the commit message;
