...
| `rc`    | external TestFlight, ship-ready                           | records they have started to rely on                     | the above **plus** Flows **7b, 7c** and **every _reachable_ out-of-band custody assertion** — four of the catalog's five rows; the key store is a written-down deferral, not a gap ([`plans/testing/crucial-flows.md`](plans/testing/crucial-flows.md) → _Where these run on mobile_) |
