...
d2dd420) was **Android 3/3 and iOS 2/3, with every flow on both platforms green** — the one
red was Maestro failing to open a session before any flow ran (item 0). Every cause has been the harness or a flow meeting a
...
5. **The build cache never saves** ("Cache save failed" on every job). Three causes have been
   ...
   **size is real** — 35546805924 reported ~2 GB of paths per iOS job *without* `DerivedData`,
   ...
   now**; the others restore. If that still fails, the next lever is dropping
