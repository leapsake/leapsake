...
[`plans/encryption/model.md`](../../plans/encryption/model.md); what _sharing_ and the _web
app_ will later do with these keys is still design, and stays there too.
...
## Custody states — the two ways a client can exist _(decided 2026-07-26)_
...
reinstall, migration, repair, or the signing-identity change below) the _only_ way back was a
...
also registered with a relay. This matters: local-only and synced users have _identical_
custody, so "start syncing later" adds a relay binding rather than a new ritual.

> **The state names the account, not the file** _(renamed 2026-07-31)_. Everything below
> _Custody_ in that table is a **consequence** of the row above it, not part of the
...
> exactly this implication. When that lands, only the _Store on disk_ row changes — the state
...
vocabularies **on purpose**: an earlier single word ("Open") collided with the verb _open_,
...
- **Authenticated ⇒ encrypted is true today, and is a consequence, not a definition** — the
  opt-out above is what severs it. Code asking _"how do I open this file?"_ must read the
...
- **Degraded** is Authenticated _and_ unlocked _and_ still broken: the device holds its db-key
  but cannot prove the account's master key. If the axes were one enum it would have nowhere
  to live.

**The ELI5 test** — _can you use the app right now without typing anything?_ Unauthenticated:
...
| **No**  | fresh install | stays Unauthenticated. An account is _invited_ later, never demanded                                                                                           |
...
**Username + password is required to turn encryption on** _(decided 2026-07-27)_. The
alternative considered and rejected was a phrase-only "accountless encryption": it makes the
recovery phrase the _primary_ credential — reviving the exact unfamiliar ritual this decision
...
> on this device is _accountless_ in every sense a user cares about: no email, no server,
...
> **The copy must promise access, not safety.** A local account protects against _this device
> losing its security settings_; it does **not** protect against a lost or broken device. Say
...
disk in the clear. The conversion writes a _new_ encrypted file and deletes the original, so
no plaintext survives inside the live database — but deleted bytes can linger in free space,
and on SSDs cannot be reliably erased. **Accepted** _(owner, 2026-07-26)_: the window is
...
## The two exits from local-only _(built 2026-08-08)_

> **A local store — Unauthenticated _or_ Authenticated — must always be mergeable into an
> authenticated synced account.** _(owner, 2026-08-02.)_
...
| **Sign out**       | a user **action** → the Locked state. _Identical for local-only and synced users_                                            |
...
state.** Both users get the same promise — _nobody can see my data on this device anymore_ —
and the only difference (whether the bytes remain, encrypted) is invisible to that intent. One
honest line covers it for a local-only user: _"Your data stays on this device, encrypted.
You'll need your password to get back in."_
...
> **"Make local-only" was cut** _(owner, 2026-07-28)_. There used to be a third,
...
> question now that binding _does_ exist, since a released handle is the one thing the relay's
...
**v0.1 ships the deliberate half only** _(scope decision, owner, 2026-07-27)_. Sign out and
...
Forgetting an account on its **last remaining device** is functionally a deletion _unless some
server durably holds a copy_. Two facts make this sharper than it first looks:

- The relay is designed to be **disposable** — devices self-heal it — so a relay is explicitly
  _not_ a backup ([`apps/server`](../../apps/server/README.md)).
- **Not every relay will offer backup.** Someone has to host that data; a self-hoster may
  choose to, and many will not. It is a property of _who is hosting_, so it is a **relay
...
_both_ destructive confirmations on mobile's Data screen — Forget account and the accountless
...
Build this as a _check_, not a hardcoded string: when server-side backup ships, alarming copy
...
| **db-key**            | client, at account creation           | the whole-DB at-rest key; read from a sidecar _before_ the store opens                          |
...
| **Principal keypair** | per server integration                | a constrained reader (Alexa / CardDAV / hosted link) you wrap _specific_ CKs to                 |
| **Session key**       | server, at SSR login                  | wraps MK for one trusted SSR session                                                            |

The **invariant** through every phase: a server never holds an _unwrapped_ MK at rest, and
...
> _Ledger: empty._ Wiping the keychain costs this user nothing, and the file opens anywhere it
...
necessity: they are read _before_ the database can open. Then converts the store and destroys
the plaintext original.

> _Ledger:_ MK and db-key each reachable by enclave, password, or phrase. The user holds two
...
> _Ledger:_ unchanged from 0.5, plus the relay's copy of `wrap(MK, KEK)` + verifier + salt.
...
into memory → mints its _own_ enclave key and db-key, adds `wrap(MK, device-2 enclave)`, and
caches the unlock.

> _Ledger:_ MK reachable from either device's enclave, the RK, or the KEK. Two devices, one
> account, relay still blind.

**Degraded — a device that holds the account but cannot prove its master key** _(decided
2026-07-29)_. Not a phase but a condition any Authenticated device can land in, and the answer
...
- **It is visible and has one exit** — the unlock gate. Signing out lands there, where the
  _other_ door is one action away (a phrase door is untouched by a broken password door and
  vice versa), and the next open repairs the enclave from it and rewinds both sync watermarks
  so the records lost in each direction are re-offered once. What the notice may _claim_ has
...
> _Ledger:_ db-key reachable; MK reachable only from the doors, not from this enclave. Local
...
> choice — see _Pre-v0.1 latitude_ in [`AGENTS.md`](../../AGENTS.md).
...
## The product model this serves _(stated 2026-07-11)_
...
- A **user** uses Leapsake on **one-to-many clients**. A **client** hosts **one unauthenticated
  user OR multiple authenticated users** — never multiple _unauthenticated_ users. Each
...
- **Forgetting the last device is treated as deletion unless a server durably holds a copy** —
  see _Forgetting the last device_ above.
...
What that costs depends entirely on the custody model, which is why _encryption follows custody_
...
Under the _old_ default — encrypt always — the same event dropped every user into a
...
- **Key-bearing objects never reach a log or a crash reporter.** The master key is a plaintext
  ...
  and transient wrap keys _are_ wiped after use, since they are derived, used once, and cost
  nothing to clear.
...
- **Sign out clears the db-key _and_ the recovery key, and never `device-id` or `enclave`.** The
...
- **Rotating the phrase takes the master key from the password door, never the enclave.** A
  ...
  (`flushPendingRecoveryEscrow` in core), and until then the _old_ phrase still recovers the
