...
The package owns only the math. *Where* keys live is the `KeyStore` port (its
platform adapters are in the apps); *how* keys are wrapped for which principal is
...
accounts/rows). The design audit that fixed them — and the rationale for *why*
...
> `security-review.md` audit is an *internal design review*, not a third-party
