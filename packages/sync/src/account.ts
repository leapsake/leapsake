...
/**
 * Assemble the production {@link SyncEngine} for an account: the caller's
 * allowlist, durable {@link createSyncStateRepo}
...
export function createAccountSyncEngine(opts: {
  ...
  /** The allowlist of what may leave the device. Injected because *what syncs*
   *  is a decision the composition root owns — `syncableRepos` in
   *  `@leapsake/core` — not one this package should make. */
