...
/// * `depth` - Optional maximum depth to descend. None means unlimited depth.
///             Depth 0 = only files in the root directories, Depth 1 = one level of subdirectories, etc.
///
/// # Returns
...
/// ```ignore
/// // Get all files
/// let files = get_files(&["/path/to/dir"], None, None)?;
///
/// // Get all .txt files with depth limit of 2
/// let files = get_files(&["/path/to/dir"], Some(&["txt"]), Some(2))?;
/// ```
pub fn get_files<P: AsRef<Path>>(
    ...
    dirs.iter()
        .filter_map(|dir| {
            let dir = dir.as_ref();
            if dir.exists() {
                Some(collect_files_recursive(dir, extensions, depth, 0))
            } else {
                None
            }
        })
...
/// let names = get_files_with(&["/path/to/dir"], None, None, |p: &PathBuf| {
    ...
    F: Fn(&PathBuf) -> T + Copy,
{
    dirs.iter()
        .filter_map(|dir| {
            let dir = dir.as_ref();
            if dir.exists() {
                Some(collect_files_with_callback(dir, extensions, depth, 0, callback))
            } else {
                None
            }
        })
        ...
        .map(|entry| {
            ...
            if path.is_dir() {
                ...
                if should_descend {
                    ...
                    collect_files_recursive(&path, extensions, max_depth, current_depth + 1)
                } else {
                    Ok(vec![])
                }
    ...
    entries
        .map(|entry| {
            ...
            if path.is_dir() {
                ...
                if should_descend {
                    // Recursively process subdirectories (follows symlinks)
                    collect_files_with_callback(&path, extensions, max_depth, current_depth + 1, callback)
                } else {
                    Ok(vec![])
                }
            } else if path.is_file() {
...
mod tests {
    ...
    fn test_get_files_no_filter() {
        ...
        let files = get_files(&[temp_path], None, None).unwrap();
    ...
    fn test_get_files_with_extension_filter() {
        ...
        let files = get_files(&[temp_path], Some(&["txt"]), None).unwrap();
    ...
    fn test_get_files_skips_nonexistent_dirs() {
        ...
        let files = get_files(&[temp_path, &nonexistent], None, None).unwrap();
    ...
    fn test_get_files_multiple_directories() {
        ...
        let files = get_files(&[temp_dir1.path(), temp_dir2.path()], None, None).unwrap();
        assert_eq!(files.len(), 2);
    }

    #[test]
    fn test_get_files_with_depth_limit() {
        let temp_dir = TempDir::new().unwrap();
        let temp_path = temp_dir.path();

        // Create nested structure
        fs::File::create(temp_path.join("root.txt")).unwrap();
        fs::create_dir(temp_path.join("depth1")).unwrap();
        fs::File::create(temp_path.join("depth1/file1.txt")).unwrap();
        fs::create_dir(temp_path.join("depth1/depth2")).unwrap();
        fs::File::create(temp_path.join("depth1/depth2/file2.txt")).unwrap();

        // Depth 0: only root files
        let files = get_files(&[temp_path], None, Some(0)).unwrap();
        assert_eq!(files.len(), 1);

        // Depth 1: root + one level deep
        let files = get_files(&[temp_path], None, Some(1)).unwrap();
        assert_eq!(files.len(), 2);

        // Depth 2: root + two levels deep
        let files = get_files(&[temp_path], None, Some(2)).unwrap();
        assert_eq!(files.len(), 3);
    ...
    fn test_get_files_with_callback() {
        ...
        let file_names: Vec<String> = get_files_with(&[temp_path], None, None, callback).unwrap();
    ...
    fn test_get_files_with_transformation() {
        ...
        let lengths: Vec<usize> = get_files_with(&[temp_path], None, None, callback).unwrap();
